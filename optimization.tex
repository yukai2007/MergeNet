...
\begin{itemize}
    ...
    \item \texttt{center} 模式只保留 no-grad 质心，能减少显存，但不等价于完整 source matrix，因为 Perceiver-style cross attention 不再获得 $\log S$ bias。当前 CLS 分类路径已经使用 \texttt{token\_center} 做排序；若要进一步增强，需要补 center-band bias。
\end{itemize}
