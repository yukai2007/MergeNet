...
\begin{enumerate}
    ...
    \item \textbf{Source Trace 更新：} \texttt{matrix} 模式维护 source matrix，用于后续质心计算和 Perceiver-style 交叉注意力的 bias；\texttt{center} 模式只维护 no-grad center-of-mass，不构造 dense source matrix，也不提供 source-aware bias。需要注意，当前 CLS 分类训练路径尚未消费 \texttt{token\_center}，因此 \texttt{center} 脚本更准确地说是关闭 dense source/bias 的轻量下界，而不是完整 center-guided 版本。
\end{enumerate}

\subsection{Top-N 选择与 Perceiver 聚合}

合并完成后，根据 token 质量 $v$ 选择质量最大的 $L_{trace} = N_p - M_{local} + 1 = 197$ 个 token（含 CLS），按质心排序后通过 Perceiver-style 交叉注意力生成进入 Latent Encoder 的序列。若 \texttt{source\_trace\_mode=matrix}，cross attention bias 由 $\log(S+\epsilon)$ 构造；若为 \texttt{center}，当前 CLS 分类路径实际不提供 source bias，并在无 source matrix 时退回按原索引排序。
