...
\begin{enumerate}
    ...
    \item \textbf{第二步（Dual AB 双分支训练）：} 在分支 A 基础上引入带压缩能力的分支 B，通过 T1 Joint / T2 Alternate / T3 Staged 对照实验，以及当前推荐的 T4 EM-style Local/Latent 坐标训练，探索最优的双分支协同方式。
\end{enumerate}
...
\begin{itemize}
    ...
    \item \texttt{trainer/classification/2000s2\_mergenet\_source\_matrix\_off\_robust.sh} — 第二步 T4：EM-style Local/Latent 坐标训练（当前推荐主线）
\end{itemize}
...
%=============================================================================
\section{T4：EM-style Local/Latent 坐标训练（推荐主线）}
%=============================================================================

\subsection{核心判断}

这里的 ``EM'' 不是 Exponential Moving Average，而是类似 EM / coordinate descent 的交替优化：固定一部分表征模块，训练另一部分表征模块。当前最稳妥的顺序是 \textbf{local-first}：

\begin{enumerate}
    \item \textbf{Local phase：} 固定 Latent Encoder，训练 Local Encoder、DTEM metric/routing、分类头、fusion 和 cross-attention。
    \item \textbf{Latent phase：} 固定 Local Encoder 与 DTEM metric/routing，训练 Latent Encoder、分类头、fusion 和 cross-attention。
\end{enumerate}

采用 local-first 的原因是：Branch A 的强 checkpoint 来自无压缩路径，虽然能提供稳定分类特征，但没有真正训练 Branch B 的压缩路由。若一开始训练 latent，而 local/DTEM 路由仍不稳定，Latent Encoder 会适配一个持续漂移的压缩输入；相反，先固定 latent，让 local/DTEM 学会把压缩后的 token 送入稳定的 latent 空间，再让 latent 适配已经成形的压缩分布，训练条件更稳。

\subsection{参数分组}

\begin{table}[h]
\centering
\caption{T4 EM-style optimizer 参数分组}
\begin{tabular}{l l l}
\toprule
\textbf{组别} & \textbf{参数范围} & \textbf{训练行为} \\
\midrule
\texttt{local} & \texttt{local.vit.blocks}, \texttt{local.metric\_layers} & 仅 Local phase 更新 \\
\texttt{latent} & \texttt{latent} encoder 参数 & 仅 Latent phase 更新 \\
\texttt{always} & patch/head/fusion/cross-attention/bridge 等其他参数 & 两个 phase 都更新 \\
\bottomrule
\end{tabular}
\end{table}

实现上，\texttt{in1k\_trainer.py} 会在 optimizer 创建后把单个参数组拆成上述三组，并通过每组的 \texttt{lr\_scale} 控制是否更新。冻结组的 \texttt{lr\_scale=0}，因此有效学习率为 $0$；已有的 gradient clipping 路径会跳过 \texttt{lr\_scale=0} 的参数，避免被冻结组的历史梯度干扰当前 phase 的全局范数。
...
\begin{verbatim}
trainer/classification/2000s2_mergenet_source_matrix_off_robust.sh
\end{verbatim}
...
\texttt{em\_local\_latent\_schedule} & \texttt{true} & 启用坐标训练 \\
\texttt{em\_first\_phase} & \texttt{local} & 先训练 local/DTEM 路由 \\
\texttt{em\_local\_epochs} & $10$ & 每轮 Local phase 长度 \\
\texttt{em\_latent\_epochs} & $20$ & 每轮 Latent phase 长度 \\
\texttt{fused\_loss\_start\_epoch} & $60$ & 两个完整 EM cycle 后再强化 fusion \\
...
\[
    \text{phase}(e) =
    \begin{cases}
        \text{local} & \text{if cycle 内落在前 } E_{local} \text{ 个 epoch} \\
        \text{latent} & \text{otherwise}
    \end{cases}
\]
其中 $E_{local}=10$，$E_{latent}=20$，cycle 长度为 $30$。Local phase 中：
\[
    \texttt{lr\_scale}_{local}=1,\quad
    \texttt{lr\_scale}_{latent}=0,\quad
    \texttt{lr\_scale}_{always}=1
\]
Latent phase 中：
\[
    \texttt{lr\_scale}_{local}=0,\quad
    \texttt{lr\_scale}_{latent}=1,\quad
    \texttt{lr\_scale}_{always}=1
\]
...
\begin{itemize}
    \item \textbf{相比 T1 Joint：} T1 会让 local 路由与 latent 表征同时漂移，早期 Branch B 压缩路径容易学习不充分；T4 把两个难点拆开，降低相互干扰。
    \item \textbf{相比 T2 Alternate：} 当前 Alternate 每步只走 A 或 B，fusion head 不参与训练；T4 始终使用 \texttt{active\_branch='both'} 的 joint 前向，fusion/head/cross-attention 作为 \texttt{always} 组持续更新。
    \item \textbf{相比 T3 Staged：} 若已经有强 Branch A checkpoint，继续 A-only 阶段收益有限，反而推迟 Branch B 的压缩学习；T4 从第 0 个 epoch 就让 B 的 local/DTEM 路由进入有效训练。
    \item \textbf{能保证的部分：} 每个核心模块都有非零学习率的训练窗口，local/DTEM、latent、head/fusion 都不会被遗漏；冻结窗口不会被 optimizer 的有效学习率更新。
    \item \textbf{不能数学保证的部分：} 任何调度都不能保证每个模块 ``完全学好'' 或全局收敛。T4 的目标是消除当前实现中已知的不训练/互相拖拽问题，并给每个模块提供更干净、更稳定的学习信号。
\end{itemize}
...
T4 不改变每步双分支 joint 前向，而是在 optimizer 层面做 local/latent 坐标冻结，因此它的显存接近 T1，但有效训练覆盖更清晰：
...
\begin{tabular}{l l}
\toprule
\textbf{维度} & \textbf{T4 EM-style Local/Latent} \\
\midrule
每步前向 & 始终双分支 \texttt{active\_branch='both'} \\
融合头训练 & 始终参与，fusion CE 延迟到第 $60$ epoch 后平滑引入 \\
Local/DTEM 更新 & 每个 cycle 前 $10$ epoch 更新，Latent 冻结 \\
Latent 更新 & 每个 cycle 后 $20$ epoch 更新，Local/DTEM 冻结 \\
Always 组更新 & head、fusion、cross-attention、bridge 等每步更新 \\
...
bash trainer/classification/2000s2_mergenet_source_matrix_off_robust.sh
    \end{verbatim}
